...
function storageToggleRow(bucket, key, label, subtitle=""){
  ...
  return `
    <div class="row ${rowClass}" style="margin-bottom:10px">
...
function renderStorageChecklist(bucket){
  ...
  return `
    ...
    ${storageToggleRow("supplements","bathroom"," / Skincare station")}
...
function schedClass(title, meta){
  const t = String(title||"").toLowerCase();
  const m = String(meta||"").toLowerCase();
  const has = (s) => t.includes(s) || m.includes(s);

  if(has("sleep") || has("bed")) return "entry-sleep";
  if(has("workout") || has("gym")) return "entry-workout";
  if(has("shake") || has("breakfast") || has("lunch") || has("dinner") || has("snack") || has("meal")) return "entry-meal";
  if(has("supp") || has("vitamin") || has("magnesium") || has("creatine") || has("fish oil") || has("multivitamin") || has("collagen") || has("msm") || has("iron")) return "entry-supp";
  if(has("shower") || has("groom") || has("skincare") || has("brush")) return "entry-hygiene";
  if(has("work")) return "entry-work";
  if(has("personal")) return "entry-personal";
  return "entry-other";
}

function checkRow(id, title, meta, extraHtml="", rowClass=""){
...
async function renderDay(day){
  ...
  const rows = items.map((it, idx) => {
    ...
    return checkRow(id, it[0], it[1], "", schedClass(it[0], it[1]));
  }).join("");
...
async function renderGrocery(){
  ...
  const rows = data.grocery.map((name, idx) => {
    ...
    const extra = `
      <div class="priceRow">
        <label class="itemMeta" style="min-width:90px">On hand</label>
        <div class="pill" style="display:inline-flex;align-items:center;justify-content:center;min-width:48px;height:32px">${getQty("grocery", `grocQty:${idx}`)}</div>
      </div>
      <div class="priceRow">
  ...
  function section(title, arr, prefix){
    const rows = arr.map((name, idx) => {
      ...
      const extra = `
        <div class="priceRow">
          <label class="itemMeta" style="min-width:90px">On hand</label>
          <div class="pill" style="display:inline-flex;align-items:center;justify-content:center;min-width:48px;height:32px">${getQty("supplements", `${prefix}Qty:${idx}`)}</div>
        </div>
        <div class="priceRow">
          <label class="itemMeta" style="min-width:90px">Price ($)</label>
...
function wireSuppPrices(){
  const totalEls = ["#sTotal"].map(sel => document.querySelector(sel)).filter(Boolean);
